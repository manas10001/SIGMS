...
%>
<button type="button" onclick="location.href = 'manageGroups.jsp?sigid=<%=session.getAttribute("sigid")%>'" style="top:70vh;" class="hoverbtn btn btn-success shadow-lg p-3 mb-5 rounded" >
  Manage Group
</button>
<%}%>
...
%>
<button type="button" onclick="location.href = 'LeaveGroup?sigid=<%=session.getAttribute("sigid").toString()%>&uid=<%=session.getAttribute("uid").toString()%>&page=gd';" style="top:90vh;" class="hoverbtn btn btn-danger shadow-lg p-3 mb-5 rounded" >
  Leave Group
</button>
<%
...
<center>
    ...
    <%
        Statement ststat = con.createStatement();
        ResultSet rs; 
        rs = ststat.executeQuery("select count(user_id) as count from group_members where group_id='"+session.getAttribute("sigid")+"' and WEEK(joined_on)=WEEK(now())-0");
        rs.next();
        String one = rs.getString("count");
        rs = null;
        
        rs = ststat.executeQuery("select count(user_id) as count from group_members where group_id='"+session.getAttribute("sigid")+"' and WEEK(joined_on)=WEEK(now())-1");
        rs.next();
        String two = rs.getString("count");
        rs = null;
        
        rs = ststat.executeQuery("select count(user_id) as count from group_members where group_id='"+session.getAttribute("sigid")+"' and WEEK(joined_on)=WEEK(now())-2");
        rs.next();
        String three = rs.getString("count");
        rs = null;
        
        rs = ststat.executeQuery("select count(user_id) as count from group_members where group_id='"+session.getAttribute("sigid")+"' and WEEK(joined_on)=WEEK(now())-3");
        rs.next();
        String four = rs.getString("count");
        rs = null;
        
        rs = ststat.executeQuery("select count(user_id) as count from group_members where group_id='"+session.getAttribute("sigid")+"' and WEEK(joined_on)=WEEK(now())-4");
        rs.next();
        String five = rs.getString("count");
    %>
...
var myChart = new Chart(ctx, {
    ...
    data: {
        ...
        datasets: [{
            ...
            borderWidth: 2,
            borderColor: ['Red', 'Blue', 'Yellow','Green','Cyan'],
            hoverBorderColor: ['Red', 'Blue', 'Yellow','Green','Cyan'],
            hoverBackgroundColor: ['Red', 'Blue', 'Yellow','Green','Cyan']
        }]
    },
