    ...
    try{
        ...
        if(!rsgrp.next())   //if there is nothing in resultset that mens there is no such sig
        {
            ...
            if(rsmem.next()){
                ...
                //querying the database to find whether a user is admin or moderator of this group
                Statement stev = con.createStatement();

                ResultSet rsauth = stev.executeQuery("select designation from `group_members` where group_id = '"+rsgrp.getString("group_id")+"' and user_id = '"+session.getAttribute("uid").toString()+"' and designation in('admin','moderator') ");
                if(rsauth.next())
                    canAddEvent = true;
            }
                    ...
                    <div class="col-sm-3">
                        <div class="card shadow bg-white rounded features-card">
                            ...
                            <div class="card-footer text-muted text-center" style="max-height: 15vh">
                                ...
                                <button class="btn btn-warning" onclick="location.href='Mailer?msg=Notification For event : <%=rsev.getString("event_name")%> Date: <%=rsev.getString("date")%> &page=groupContentEvents.jsp&eid=<%=rsev.getString("event_id")%>'">Get Notification</button>
...
%>
                    <!--new event section-->
                    <button type="button" style="bottom:1px;right:25px;position:fixed;" class="btn btn-success btn-lg shadow-lg p-3 mb-5 rounded" data-toggle="modal" data-target="#newEvent">
                      New Event
                    </button>
<%
                }else{
%>
                    <button type="button" style="bottom:1px;right:25px;position:fixed;" class="btn btn-info btn-lg p-3 mb-5 rounded" onclick="location.href = 'JoinGroup?sigid=<%=rsgrp.getString("group_id")%>';">
                      You Have to be a moderator to add events
                    </button>
<%
                }//end can add event blk
            }else{
%>
...
<!-- Modal Event -->
<div class="modal fade" id="newEvent" tabindex="-1" role="dialog" aria-labelledby="newPost" aria-hidden="true">
  <div class="modal-dialog modal-xl modal-dialog-centered"  role="document">
    ...
    <form action="AddEvent" method="post">
      <div class="modal-body" id="#write" style="min-height:300px;">
          <input type="text" class="event" name="evname" required placeholder="Event Name" style="width: 100%"/>
          <textarea style="min-height: 150px; width: 100%;" maxlength="500" placeholder="Write or drag content here..." required name="description"></textarea>
          <input type="text" class="event" name="venue" required placeholder="Venue" style="width:100%"/>
          <input type="date" onchange="return valDate()" name="date" placeholder="Date" class="event"  id="date"> 
      </div>
