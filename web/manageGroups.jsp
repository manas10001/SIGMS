...
//rest of the page
    ...
    try{
        ...
        if(!rsgrp.next())   //if there is nothing in resultset that mens there is no such sig
        {
            ...
            session.setAttribute("grpnme",rsgrp.getString("group_name"));
...
<div class="jumbotron">
    <div class="row align-items-center">
    	<div class="float-left clear-left" style="margin-right: 15px; max-width: 100%;" onblur="saveGrpName();"  id='grpname'>
            <h1 id='gnme'><%=rsgrp.getString("group_name")%></h1>
        </div>
...
<script>
    function submitThis(){
        ...
        var strGnmeOc = '<%=rsgrp.getString("group_name")%>';
        var strDescOc = '<%=rsgrp.getString("description")%>';
        var gid = '<%=rsgrp.getString("group_id")%>';
        
        var strGnmeNw = document.getElementById('gnme').textContent.trim();
        var strDescNw = document.getElementById('description').textContent.trim();
        
        if(strGnmeOc != strGnmeNw || strDescOc != strDescNw){
            
//            console.log("call servlet");

              location.href="UpdateGrpDetails?gid="+gid+"&gnme="+strGnmeNw+"&desc="+strDescNw;
        }
        return true;
//        console.log(strGnmeNw);
//        console.log(strDescNw);
    }
</script>
