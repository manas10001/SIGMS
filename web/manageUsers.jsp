    ...
    <div>
        ...
        <tbody>
            ...
            try{
                ...
                ResultSet rs = st.executeQuery("select * from `group_members` where group_id='"+session.getAttribute("sigid")+"' ");
            ...
            %>
                <tr>
                    ...
                    <th class="w-25" scope="col">
                        <%
                            if(rs.getString("designation").equals("member")){
                        %>
                                <button class="btn btn-success" onclick="location.href='mkMod?sigid=<%=session.getAttribute("sigid")%>&uid=<%=rs.getString("user_id")%>'">Make Admin</button>
                                <button class="btn btn-danger" onclick="location.href='LeaveGroup?sigid=<%=session.getAttribute("sigid")%>&uid=<%=rs.getString("user_id")%>&page=mu'">Remove from group</button>
                        <%
                            }else if(rs.getString("designation").equals("admin")){
                        %>
